...
export function PricingMatrix({
  ...
  // Filter columns: show options from current chapter and previous chapters
  const visibleColumns = useMemo(() => {
    if (!activeChapter) return [];
    const activeChapterIndex = chapters.findIndex((c) => c.id === activeChapter.id);
    return allOptions.filter((opt) => {
      const optChapterIndex = chapters.findIndex((c) => c.id === opt.chapterId);
      return optChapterIndex <= activeChapterIndex;
    });
  }, [allOptions, activeChapter, chapters]);

  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="flex flex-col w-full max-w-[90vw] max-h-[90vh] rounded-sm bg-[#e9e9e9] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        {/* Header */}
        ...
        <div className="flex gap-2 border-b border-[#999999] px-4 pt-2 overflow-x-auto bg-[#e9e9e9]">
          {chapters.map((chapter) => (
            <button
              ...
              className={`border-b-2 px-3 py-2 text-xs font-semibold uppercase tracking-wider transition-colors whitespace-nowrap ${
        ...
        <div className="flex-1 overflow-auto bg-white/20">
          <table className="w-full border-collapse text-left text-xs">
            <thead className="sticky top-0 z-10 bg-[#e9e9e9] shadow-sm">
              <tr>
                <th className="border-b border-[#999999] bg-[#e9e9e9] px-3 py-2 font-semibold text-[#111111]/50 uppercase tracking-wider sticky left-0 z-20 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">
                  Target Option (Row)
                </th>
                {visibleColumns.map((colOpt) => (
                  <th
                    key={colOpt.id}
